...
angular.module('eventsMongoApp').controller('EventsCtrl', function ($scope, $timeout, socket, CalendarEvent, Auth) {
  ...
  $scope.$watch('newEvent.edate', function handleDateChange(newValue, oldValue) {
    if (!newValue) $scope.newEvent.edate = moment().format('MMM, D dddd');
  });
  $scope.isEdit = false;
